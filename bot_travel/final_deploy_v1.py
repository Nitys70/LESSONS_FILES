...
POINTS_KEYBOARD_FROM = [
    [InlineKeyboardButton("✈️ ИЗ а/п Шереметьево", callback_data="point_а/п Шереметьево")],
    [InlineKeyboardButton("✈️ ИЗ а/п Домодедово", callback_data="point_а/п Домодедово")],
    [InlineKeyboardButton("✈️ ИЗ а/п Внуково", callback_data="point_а/п Внуково")],
    [InlineKeyboardButton("✈️ ИЗ а/п Рощино", callback_data="point_а/п Рощино")],
    [InlineKeyboardButton("🚇 ИЗ ст.м. Домодедовская", callback_data="point_ст.м. Домодедовская")],
    [InlineKeyboardButton("🚇 ИЗ ст.м. Ховрино", callback_data="point_ст.м. Ховрино")],
    [InlineKeyboardButton("🚂 ИЗ ж/д Павелецкий", callback_data="point_ж/д Павелецкий")],
    [InlineKeyboardButton("🚂 ИЗ ж/д Восточный", callback_data="point_ж/д Восточный")],
    [InlineKeyboardButton("🚂 ИЗ ж/д Три вокзала", callback_data="point_ж/д Три вокзала")],
    [InlineKeyboardButton("🚂 ИЗ ж/д Курский", callback_data="point_ж/д Курский")],
    [InlineKeyboardButton("🚂 ИЗ ж/д Тюмень", callback_data="point_ж/д Тюмень")],
    [InlineKeyboardButton("🏨 ИЗ отель 'Салют'", callback_data="point_отель \"Салют\"")],
    [InlineKeyboardButton("🏨 ИЗ отель 'Уют'", callback_data="point_отель \"Уют\"")],
    [InlineKeyboardButton("🏨 ИЗ отель 'Ибис-Домодедово'", callback_data="point_отель \"Ибис-Домодедово\"")],
    [InlineKeyboardButton("🏨 ИЗ отель 'Виктория (Тюмень)'", callback_data="point_отель \"Виктория (Тюмень)\"")]
    ]

POINTS_KEYBOARD_TO = [
    [InlineKeyboardButton("✈️ В а/п Шереметьево", callback_data="point_а/п Шереметьево")],
    [InlineKeyboardButton("✈️ В а/п Домодедово", callback_data="point_а/п Домодедово")],
    [InlineKeyboardButton("✈️ В а/п Внуково", callback_data="point_а/п Внуково")],
    [InlineKeyboardButton("✈️ В а/п Рощино", callback_data="point_а/п Рощино")],
    [InlineKeyboardButton("🚇 В ст.м. Домодедовская", callback_data="point_ст.м. Домодедовская")],
    [InlineKeyboardButton("🚇 В ст.м. Ховрино", callback_data="point_ст.м. Ховрино")],
    [InlineKeyboardButton("🚂 В ж/д Павелецкий", callback_data="point_ж/д Павелецкий")],
    [InlineKeyboardButton("🚂 В ж/д Восточный", callback_data="point_ж/д Восточный")],
    [InlineKeyboardButton("🚂 В ж/д Три вокзала", callback_data="point_ж/д Три вокзала")],
    [InlineKeyboardButton("🚂 В ж/д Курский", callback_data="point_ж/д Курский")],
    [InlineKeyboardButton("🚂 В ж/д Тюмень", callback_data="point_ж/д Тюмень")],
    [InlineKeyboardButton("🏨 В отель 'Салют'", callback_data="point_отель \"Салют\"")],
    [InlineKeyboardButton("🏨 В отель 'Уют'", callback_data="point_отель \"Уют\"")],
    [InlineKeyboardButton("🏨 В отель 'Ибис-Домодедово'", callback_data="point_отель \"Ибис-Домодедово\"")],
    [InlineKeyboardButton("🏨 В отель 'Виктория (Тюмень)'", callback_data="point_отель \"Виктория (Тюмень)\"")]
    ]
...
MAIN_MENU = [
    ...
    ["Стартовое меню, правовая и контактная информация"],  # Если тебе важно оставить /start, пусть будет отдельно
]
def main_keyboard():
...
async def agree_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.answer()
    await query.edit_message_text("✅ Спасибо, вы согласились с Политикой обработки персональных данных, и дали своё согласие на обработку персональных данных.")
    # Теперь можно запускать основную логику (например, restart)
    # update.message = query.message  # Иногда требуется для совместимости
    await restart(update, context)        
...
async def restart(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if context.user_data.get('in_conversation'):
            # print('IN CONTEXT', context.user_data)
        ...
        # Если пользователь не в диалоге,просто запускается стартовое меню
            ...
            if not username and not phone_number:
                ...
                text = (
                    ...
                    "Вы можете создать его (это ваш Telegram-логин, например, @ivan_ivanov).\n\n"
            ...
            if phone_number:
                ...
                await reply(
                    f"👋 Привет!\n"
                    "Я помогу тебе найти попутчиков для поездок между аэропортами.\n\n"
                    "📌 Доступные команды:\n"
                ...
                # print('ELSE RESTART')
                # Есть username: сохраняем в user_data, показываем меню
...
async def pass_qnt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text(
            "Напишите, пожалуйста, важную информацию о поездке, ее увидят Ваши"
            " потенциальные попутчики. Например, если у вас есть багаж, и сколько его.\n\n"
...
async def enter_destination(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        "Просто отправьте ответным сообщением количество минут, сколько готовы подождать (например, 30)\n\n",
        reply_markup=cancel_keyboard,
        parse_mode='HTML'
    )
...
async def my_requests(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if context.user_data.get('in_conversation'):
            # print('IN CONTEXT my_requests', context.user_data)
            # ConversationHandler.END
            # await cancel(update, context)
            # print(update)
            # print(context)
        ...
        try:
            ...
            await bot.send_message(
                ...
                text=(
                    ...
                    "Пожалуйста, свяжитесь для согласования деталей."
                ),
                parse_mode='HTML'
            )
        except Exception as e:
